...
from typing import Any, Literal, TypedDict, no_type_check
...
class OnProgress:
    ...
    def __init__(
        self, upload_id: int, batch_pk: int, progress: float, metrics: Metrics
    ):
...
class OnComplete:
    ...
    def __init__(self, upload_id: int, batch_pk: int):
...
def end_upload_session(batch_pk: int, upload_session: int) -> dict[str, Any]:
    ...
    return api_client.batches_samples_end_upload_session_create(batch_pk, data)
